...
```{r check is it pure base types, include=FALSE}
x <- c(1:10)

is.object(x)
#> [1] FALSE
# FALSE means x is a pure base types object
...
* that’s the function that figures out the correct method to call, the process of method dispatch

* Similar to otype(), pryr also provides ftype() which describes the object system, if any, associated with a function.

```{r determine if a function is an S3 generic, include=FALSE}
install.packages('pryr')
library(pryr)
...
* Some S3 generics, like [, sum(), and cbind(), don’t call UseMethod() because they are implemented in C (pure base type)

* Functions that do method dispatch in C code are called **internal generics** and are documented in ?"internal generic". -> generic functions yang tidak membutuhkan method.
...
* summary :
  ...
  1. ada beberapa S3 object yang tidak bisa menggunakan call method seperti : like [, sum(), and cbind(), don’t call UseMethod() because they are implemented in C (pure base type)
  
  2. ada juga S3 object yang bisa menggunakan call method (ini disebut internal generics).
  
  3. untuk mengindetifikasi function dan method. prinsipnya di R, method adalah milik function sehingga dalam penulisannya akan selalu ''' method "." fungsi '''. contohnya :  mean.date() : mean : method dan date : fungsi
  
  4. kita bisa melihat semua method yang bisa dipanggil untuk fungsi yang dipakai dengan cara : methods('nama methods'). contoh : methods(mean)
  
  5. selain itu, kita juga bisa melihat semua list function/generics yang bisa digunakan untuk memanggil suatu method/call tertentu. dengan cara, methods(class = "nama_method"). contoh : methods (class = ts) atau methods(class = data.frame)

```{r S3 generic, include=FALSE}
mean # mean have a call method
#> function (x, ...) 
#> UseMethod("mean")
#> <bytecode: 0x10a9acdc8>
#> <environment: namespace:base>

sum # sum doesnt have call method cause it is pure base type
#> function (..., na.rm = FALSE)  .Primitive("sum")

ftype(t.data.frame) # # data frame method for t() functions
#> [1] "s3"     "method"

ftype(t.test)
#> "s3"      "generic"

# 2. You can see all the methods that belong to a generic with methods():
...
# 3. You can also list all generics that have a method for a given class:
...
* S3 is a simple and ad hoc system; it has no formal definition of a class.
...
```{r create class, include=FALSE}
# Create and assign class in one step
foo <- structure(list(), class = "hallow")
...
# inherits "checking class values"
